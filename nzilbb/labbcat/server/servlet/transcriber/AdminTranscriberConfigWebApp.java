...
import nzilbb.ag.stt.util.RequestException;
 ...
 * <p> This servlet manages post-installation configuration of transcribers, by presenting the
 * given transcriber's 'config' web-app, if there is one, and accepting a <q>setConfig</q>
 ...
 * <tt>/api/admin/transcriber/config/<var>transcriberId</var>/</tt>
 * <p> The resulting HTML document will then implement the web-app by requesting resources
 * as required.
 * <p> Once ready (or if the transcriber implements no 'config' web-app) a request to <br>
 * <tt>/api/admin/transcriber/config/<var>transcriberId</var>/setConfig</tt>
 * <br>... with the body of the request representing the transcriber configuration, if any.
 * <p> The response to the <tt>setConfig</tt> request will be an HTML document that
 * renders a progress bar, and manages updating progress as the transcriber completes its
 * configuration.
 * @author Robert Fromont robert@fromont.net.nz
 */
@WebServlet({"/admin/transcriber/config/*", "/api/admin/transcriber/config/*"})
@RequiredRole("admin")
public class AdminTranscriberConfigWebApp extends LabbcatServlet {

   HashMap<String,TranscriberDescriptor> activeTranscribers
   = new HashMap<String,TranscriberDescriptor>();

   /**
    * Default constructor.
    */
   public AdminTranscriberConfigWebApp() {
   } // end of constructor
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
         ...
         try {
            ...
            String transcriberId = path.group(1);
            String resource = path.group(2);
            if (resource.equals("/")) resource = "/index.html";
            log("transcriberId " + transcriberId + " resource " + resource);

            // get transcriber descriptor - the same instance as last time if possible
            TranscriberDescriptor newDescriptor = store.getTranscriberDescriptor(transcriberId);
            log("new descriptor " + (newDescriptor==null?"null":newDescriptor.getVersion()));
            TranscriberDescriptor descriptor = activeTranscribers.get(transcriberId);
            ...
            if (descriptor == null // haven't got one of these yet
               ...
               activeTranscribers.put(transcriberId, descriptor);
            ...
            if (!descriptor.hasConfigWebapp()
               ...
               log("no config webapp " + transcriberId + " " + resource);
               response.setStatus(HttpServletResponse.SC_NOT_FOUND);
               return;
            }

            final Transcriber transcriber = descriptor.getInstance();
            try {
            ...
            if (resource.equals("/getSchema")) {
               stream = new ByteArrayInputStream(
                  transcriber.getSchema().toJson().toString().getBytes());
               ...
               new Thread(new Runnable() {
                     public void run() {
                        try {
                           ...
                           File f = new File(
                              transcriber.getWorkingDirectory(),
                              transcriber.getTranscriberId() + ".cfg");
                           try {
                              ...
                              System.err.println(
                                 "Could not write configuration file "+f.getPath()+": "+exception);
               ...
               if (resource.indexOf('.') > 0) {
                  if (resource.equals("/util.js")) {
                     // built-in utilities script
                  ...
                  try {
                     stream = new RequestRouter(transcriber).request(
...
} // end of class AdminTranscriberConfigWebApp
