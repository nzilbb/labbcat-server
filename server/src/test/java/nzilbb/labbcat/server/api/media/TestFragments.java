...
/**
 * Test the <tt>api/media/fragments</tt> endpoint.
...
public class TestFragments {
  ...
  /** Ensure sound file fragments can be extracted. */
  @Test public void mediaFragments() throws Exception {
    ...
    try {
      ...
      if (matches.length == 0) {
        ...
        File[] wavs = l.getSoundFragments(subset, null, null);
        try {
          assertEquals("files array is same size as matches array",
                       subset.length, wavs.length);
               
          for (int m = 0; m < upTo; m++) {
            assertNotNull("Non-null sized file: " + subset[m], wavs[m]);
            assertTrue("Non-zero sized file: " + subset[m], wavs[m].length() > 0);
  ...
  public static void main(String args[]) {
    org.junit.runner.JUnitCore.main("nzilbb.labbcat.server.api.media.TestFragments");
  }
}
