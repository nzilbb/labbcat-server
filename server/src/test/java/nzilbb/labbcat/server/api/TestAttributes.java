...
/**
 * Test the <tt>api/attributes</tt> endpoint.
...
public class TestAttributes {
  ...
  @Test public void attributes()
    ...
    String[] ids = l.getMatchingTranscriptIds("/BR.+/.test(id)");
    assertTrue("Some IDs are returned", ids.length > 0);
    String[] layerIds = {
      "transcript_type", "corpus", "transcript_language", "invalid_attribute"};
    
    File csv = l.getTranscriptAttributes(ids, layerIds);
    ...
    assertEquals("Correct columns are returned, including invalid attributes",
                 "transcript,transcript_type,corpus,transcript_language,invalid_attribute",
                 lines[0]);
    csv.delete();
  }

  /** Test invalid transcript request */
  @Test public void invalidTranscript() throws Exception {
    String[] layerIds = {
      "transcript_type", "corpus", "transcript_language", "invalid_attribute"};
    String[] invalidId = { "invalid-id" };
    File csv = l.getTranscriptAttributes(invalidId, layerIds);
    assertNotNull("File returned", csv);
    assertTrue("File exists", csv.exists());
    String data = IO.InputStreamToString(new FileInputStream(csv));
    String[] lines = data.split("\n");
  ...
  public static void main(String args[]) {
    org.junit.runner.JUnitCore.main("nzilbb.labbcat.server.api.TestAttributes");
  }
}
